...
annMin=dplyr::group_by(SalWTI,Year) %>% summarize(annMin=mean(TMIN))

SalWTI=full_join(SalWTI,annMin)
SalWTI$detMin=0
for (i in 1:nrow(SalWTI)) {
  SalWTI[i,13]=SalWTI[i,5]/SalWTI[i,12]
}
...
SalDet[1,24]=0
SalDet[2,24]=SalDet[1,21]
SalDet[1,25]=0
SalDet[2,25]=0

for (i in 3:nrow(SalDet)) {
  SalDet[i,24]=SalDet[(i-1),21]
  SalDet[i,25]=SalDet[(i-2),21]
}
...
for (i in 1:nrow(SalDet)) {
  SalDet[i,26]=(SalDet[i,15]-SalDet[i,2])/SalDet[i,3] ##tmax
  SalDet[i,27]=(SalDet[i,23]-SalDet[i,4])/SalDet[i,5] ##tmin
  SalDet[i,28]=(SalDet[i,12]-SalDet[i,6])/SalDet[i,7] ##wind
  SalDet[i,29]=(SalDet[i,20]-SalDet[i,8])/SalDet[i,9] ##irrad
  SalDet[i,30]=(SalDet[i,14]-SalDet[i,10])/SalDet[i,11] ##tavg
}

SalDet$type=0 ##type = seasonal ish classification (wet/dry/in between)

for (i in 1:nrow(SalDet)) { 
  if (SalDet[i,18]<4 | SalDet[i,18]>10 ) {
    SalDet[i,31]=5
  } else if (SalDet[i,18]==7 | SalDet[i,18]==8 | SalDet[i,18]==6) {
    SalDet[i,31]=0
...
for (i in 1:nrow(SalDet)) {
  if (SalDet[i,18]==1) {
    ...
    SalDet[i,32]= (wet1+1)/(wet1+vwet1+2) #wet
    SalDet[i,33]= (vwet1+1)/(wet1+vwet1+2) #v wet
  } else if (SalDet[i,18]==2) {

    SalDet[i,32]= (wet2+1)/(wet2+vwet2+2) #wet
    SalDet[i,33]= (vwet2+1)/(wet2+vwet2+2) #v wet
  } else if (SalDet[i,18]==3) {

    SalDet[i,32]= (wet3+1)/(wet3+vwet3+2) #wet
    SalDet[i,33]= (vwet3+1)/(wet3+vwet3+2) #v wet
  } else if (SalDet[i,18]==4) {

    SalDet[i,32]= (wet4+1)/(wet4+vwet4+2) #wet
    SalDet[i,33]= (vwet4+1)/(wet4+vwet4+2) #v wet
  } else if (SalDet[i,18]==5) {

    SalDet[i,32]= (wet5+1)/(wet5+vwet5+2) #wet
    SalDet[i,33]= (vwet5+1)/(wet5+vwet5+2) #v wet
  } else if (SalDet[i,18]==6) {

    SalDet[i,32]=(wet6+1)/(wet6+vwet6+2) #wet
    SalDet[i,33]=(vwet6+1)/(wet6+vwet6+2) #v wet
  } else if (SalDet[i,18]==7) {

    SalDet[i,32]= (wet7+1)/(wet7+vwet7+2) #wet
    SalDet[i,33]= (vwet7+1)/(wet7+vwet7+2) #v wet
  } else if (SalDet[i,18]==8) {

    SalDet[i,32]= (wet8+1)/(wet8+vwet8+2) #wet
    SalDet[i,33]= (vwet8+1)/(wet8+vwet8+2) #v wet
  } else if (SalDet[i,18]==9) {

    SalDet[i,32]= (wet9+1)/(wet9+vwet9+2) #wet
    SalDet[i,33]= (vwet9+1)/(wet9+vwet9+2) #v wet
  } else if (SalDet[i,18]==10) {

    SalDet[i,32]= (wet10+1)/(wet10+vwet10+2) #wet
    SalDet[i,33]= (vwet10+1)/(wet10+vwet10+2) #v wet
  } else if (SalDet[i,18]==11) {

    SalDet[i,32]= (wet11+1)/(wet11+vwet11+2) #wet
    SalDet[i,33]= (vwet11+1)/(wet11+vwet11+2) #v wet
  } else {

    SalDet[i,32]= (wet12+1)/(wet12+vwet12+2) #wet
    SalDet[i,33]= (vwet12+1)/(wet12+vwet12+2) #v wet
  }
}
...
write.csv(SalDet,file="C:\\Users\\rcuppari\\OneDrive\\Research\\Syn_generator_include_irr-master\\Historical_weather_analysis\\SalDet2s1997_ann.csv")

#SalDet=read.csv("C:\\Users\\rcuppari\\OneDrive\\Research\\Syn_generator_include_irr-master\\Historical_weather_analysis\\SalDet2s.csv")##detrended precipitation annually for SalDet
