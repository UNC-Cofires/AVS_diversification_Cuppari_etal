...
salem1=read.csv("C:\\Users\\rcuppari\\OneDrive\\Research\\Syn_generator_include_irr-master\\simRainSal_1997_tmin.csv")

for (i in 1:nrow(salem1)) {
  if (is.na(salem1[i,16])==TRUE) {
    salem1[i,16]=0
  }
  if (salem1[i,16]<.02) {
    salem1[i,16]=0
...
ggplot(salem1)+
  geom_histogram(aes(rain,fill="observed"),alpha=1,size=5)+facet_wrap(~Month)+
  geom_histogram(aes(simRainType,fill="simulated"),alpha=.5,size=5)+
...
salem=dplyr::filter(salem1,PRCP>.61)

paramJFM=fitdistr(salem$PRCP[salem$Month==1 |salem$Month==2|salem$Month==3],densfun="gamma")
paramApr=fitdistr(salem$PRCP[salem$Month==4],densfun="gamma")
paramMJ=fitdistr(salem$PRCP[salem$Month==5 |salem$Month==6],densfun="gamma")
paramJA=fitdistr(salem$PRCP[salem$Month==6],densfun="gamma") ##making same as june because have none
paramSep=fitdistr(salem$PRCP[salem$Month==9],densfun="gamma")
paramOct=fitdistr(salem$PRCP[salem$Month==10],densfun="gamma")
paramND=fitdistr(salem$PRCP[salem$Month==11|salem$Month==12],densfun="gamma")

salem1$simP=0
for (i in 1:nrow(salem1)) {
  if (salem1[i,37]==2) {
    if (salem1[i,21]==1 |salem1[i,21]==2|salem1[i,21]==3) {
      salem1[i,38]=rgammatr(1,paramJFM$estimate["shape"],paramJFM$estimate["rate"],c(.60,3.5))
    }
    else if (salem1[i,21]==4) {
      salem1[i,38]=rgammatr(1,paramApr$estimate["shape"],paramApr$estimate["rate"],c(.60,3.5))
    }
    else if (salem1[i,21]==5|salem1[i,21]==6) {
      salem1[i,38]=rgammatr(1,paramMJ$estimate["shape"],paramMJ$estimate["rate"],c(.60,3.5))
    }
    else if (salem1[i,21]==7|salem1[i,21]==8) {
      salem1[i,38]=rgammatr(1,paramJA$estimate["shape"],paramJA$estimate["rate"],c(.60,3.5))
    }
    else if (salem1[i,21]==9) {
      salem1[i,38]=rgammatr(1,paramSep$estimate["shape"],paramSep$estimate["rate"],c(.60,3.5))
    }
    else if (salem1[i,21]==10) {
      salem1[i,38]=rgammatr(1,paramOct$estimate["shape"],paramOct$estimate["rate"],c(.60,3.5))
    }
    else {
      salem1[i,38]=rgammatr(1,paramND$estimate["shape"],paramND$estimate["rate"],c(.60,3.5))
    }
  } else {
    salem1[i,38]=0
  }
...
###########################################################
###less intense 
salem2=dplyr::filter(salem1,PRCP<=.61)
...
for (i in 1:nrow(salem1)) {
  if (salem1[i,37]==1) {
    if (salem1[i,21]==1) {
      salem1[i,38]=rgammatr(1,paramJan2$estimate["shape"],paramJan2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==2) {
      salem1[i,38]=rgammatr(1,paramFeb2$estimate["shape"],paramFeb2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==3) {
      salem1[i,38]=rgammatr(1,paramMar2$estimate["shape"],paramMar2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==4) {
      salem1[i,38]=rgammatr(1,paramApr2$estimate["shape"],paramApr2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==5) {
      salem1[i,38]=rgammatr(1,paramMay2$estimate["shape"],paramMay2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==6) {
      salem1[i,38]=rgammatr(1,paramJun2$estimate["shape"],paramJun2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==7) {
      salem1[i,38]=rgammatr(1,paramJul2$estimate["shape"],paramJul2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==8) {
      salem1[i,38]=rgammatr(1,paramAug2$estimate["shape"],paramAug2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==9) {
      salem1[i,38]=rgammatr(1,paramSep2$estimate["shape"],paramSep2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==10) {
      salem1[i,38]=rgammatr(1,paramOct2$estimate["shape"],paramOct2$estimate["rate"],c(0,.61))
    }
    else if (salem1[i,21]==11) {
      salem1[i,38]=rgammatr(1,paramNov2$estimate["shape"],paramNov2$estimate["rate"],c(0,.61))
    }
    else {
      salem1[i,38]=rgammatr(1,paramDec2$estimate["shape"],paramDec2$estimate["rate"],c(0,.61))
    }
  } else {
...
ggplot(simMon)+
  ...
  xlab("Month")+
  ylab("Rainfall (in)")+
  #annotate("text",x=6,y=10,label="r^2=.87")+
...
dayvars=salem1[,c(16,18,19,22,17,38)]
...
feb=dplyr::filter(salem1,Month==2)
sum(feb$PRCP)
sum(feb$simP)
