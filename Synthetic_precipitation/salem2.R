...
quantile(SalWTI$PRCP,0.95,na.rm=TRUE)
...
SalWTI[,3:5]=(SalWTI[,3:5]-32)*5/9 ##conver to celsius
...
for (i in 1:nrow(SalDet)) { 
  if (SalDet[i,18]<4 | SalDet[i,18]>10 ) {
    SalDet[i,29]=10
  } else if (SalDet[i,18]==9 | SalDet[i,18]==10) {
    SalDet[i,29]=7
  } else if (SalDet[i,18]==4 | SalDet[i,18]==5 |SalDet[i,18]==6) {
    SalDet[i,29]=5
  } else {
      SalDet[i,29]=0    
  }
...
write.csv(SalDet,file="C:\\Users\\rcuppari\\OneDrive\\Research\\Syn_generator_include_irr-master\\Historical_weather_analysis\\SalDet2s1997lap.csv")
...
pre2000=dplyr::filter(SalDet,Year<2010)
pre2000=pre2000[,c(12:16,20,32)]
post=dplyr::filter(SalDet,Year>2009)
post=post[,c(12:16,20,32)]

corrplot(cor(pre2000,use="complete.obs"),method="color")
corrplot(cor(post,use="complete.obs"),method="color")
