...
train_input=train.loc[:,['AWND.1','GHI.1','TAVG.1']]
test_input=test.loc[:,['AWND.1','GHI.1','TAVG.1']]
...
hist_weath=detWeath.loc[:,['AWND.1','GHI.1','TAVG.1']]
...
plt.plot(syn_w_t_tot.iloc[1:366,6])
plt.plot(syn_w_t_tot.iloc[1:366,7])
...
plt.plot(sim_input.iloc[:,0])

##predict using simulated average temperature (Celsius already), irradiation, and wind 
knn_input=sim_input.loc[:,['syn_res_w','syn_res_ghi','syn_res_t']]
...
##doesn't look great for the simulated stuff
...
histCC=detWeath.groupby(['Month']).agg({'GHI':{'mean','std'}})
simCC=syn_weather.groupby(['month']).agg({'syn_ghi':{'mean','std'}})
plt.errorbar(np.arange(12), histCC.iloc[:,0], histCC.iloc[:,1], fmt='bv',lw=3)
plt.errorbar(np.arange(12), simCC.iloc[:,0], simCC.iloc[:,1], fmt='ro',lw=3,alpha=.5)
 
samp2=detWeath.iloc[274:1274,5]
syn2=syn_weather.iloc[0:1000,1]
plt.plot(samp2.reset_index(drop=True))
plt.plot(syn2.reset_index(drop=True))
