...
# and last)
    ...
    R = Residuals.loc[:,'SALEM_T':]
    
    R=R.iloc[0:len(Solar_R),:]
    ...
    E= np.exp(E)-5 ##un transform (log transformation) to get actual residual 
    ...
    sim_weather=np.zeros((sim_days,fields-8))
    n=0
    
    # for each simulated day
    for i in range(0,sim_days):
        for j in range(0,fields-8):
            ...
            if j%2==0: #means it's a temperature column 
                pass
            else:
                sim_weather[i,j] = np.max((sim_weather[i,j],0)) ##can't have negative wind speeds
        n=n+1
        if n >= 365: ##restart day count
            n=0
    
    n=0
    sim_irr=np.zeros((sim_days,8))
    for i in range(0,sim_days):
        for j in range(0,8):
...
#    df_sim.to_csv('Synthetic_weather/synthetic_weather_data_1.csv')
    ...
    df_sim2.to_csv('Synthetic_weather\\synthetic_weather_data1200_2.csv')
    
    df_sim_irr=pd.DataFrame(sim_irr2)
    df_sim_irr.columns=headers2
    df_sim_irr.to_csv('Synthetic_weather\\synthetic_irradiance_data1200_2.csv')
